...
if __name__ == '__main__':
    ...
    param_dict = load_checkpoint("/data/wangjiaqi/code/mindspore/TesNet-mindspore_2/vgg19_warm_ckpt/vgg19_warm-4_5619.ckpt")
    ...
    joint_lr = nn.cosine_decay_lr(min_lr=0.0, max_lr=3e-3,
                                total_step=(config.num_train_epochs-config.num_warm_epochs) * step_size, step_per_epoch=step_size,
                                decay_epoch=(config.num_train_epochs-config.num_warm_epochs))
    joint_optimizer = nn.Momentum(joint_optimizer_specs,joint_lr,momentum=0.9)
    ...
    if config.save_checkpoint:  #
        ...
        ckpt_cb = ModelCheckpoint(prefix="{}_joint".format(config.base_architecture), directory="{}/{}_joint_ckpt".format(config.save_checkpoint_path,config.base_architecture), config=config_ck)
        cb += [ckpt_cb]
    joint(tesnet_model)
    joint_model.train((config.num_train_epochs-config.num_warm_epochs), train_dataset, callbacks=cb, sink_size=step_size, dataset_sink_mode=False)
